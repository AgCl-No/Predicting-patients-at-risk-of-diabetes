...
# 2. 计算连续型特征与响应变量 diabetes 的相关性
# 选择所有连续型特征（age, weight, height, bmi）
continuous_features <- c("age", "weight", "height", "bmi")

# 计算每个连续型特征与响应变量 diabetes 的皮尔逊相关系数
# 确保 diabetes 列是数值型
diabetes_cleaned$diabetes <- as.numeric(as.factor(diabetes_cleaned$diabetes)) - 1

# 计算每个连续型特征与响应变量 diabetes 的皮尔逊相关系数
correlation_with_diabetes <- sapply(continuous_features, function(x) cor(diabetes_cleaned[[x]], diabetes_cleaned$diabetes, use = "complete.obs"))
print("Continuous feature correlation with diabetes:")
print(correlation_with_diabetes)


# 3. 计算类别型特征与响应变量 diabetes 的相关性
# 选择所有类别型特征（smoker, sex, coronary_heart_disease, hypertension, heart_condition, cancer, family_history_diabetes）
categorical_features <- c("smoker", "sex", "coronary_heart_disease", "hypertension", "heart_condition", "cancer", "family_history_diabetes")

# 使用点二列相关系数（Point-Biserial Correlation）计算每个类别变量与响应变量 diabetes 之间的相关性
categorical_correlation <- sapply(categorical_features, function(x) cor.test(as.numeric(diabetes_cleaned[[x]]), diabetes_cleaned$diabetes)$estimate)
print("Categorical feature correlation with diabetes:")
print(categorical_correlation)

# 4. 将所有相关性结果汇总为一个数据框
correlation_results <- data.frame(
  Feature = c(continuous_features, categorical_features),
  Correlation = c(correlation_with_diabetes, categorical_correlation)
)

# 打印结果
print("Correlation results for all features with diabetes:")
print(correlation_results)

# 5. 可视化相关性结果：条形图展示每个特征与 diabetes 的相关性
ggplot(correlation_results, aes(x = Feature, y = Correlation, fill = Correlation)) + 
  geom_bar(stat = "identity", show.legend = FALSE) +
  theme_minimal() +
  coord_flip() +
  labs(title = "Feature-Response Variable Correlation", x = "Features", y = "Correlation with Diabetes") +
  scale_fill_gradientn(colors = c("#FFFAF0", "#FF5733", "#C70039")) # 设置渐变颜色
...
ggcorrplot(correlation_matrix, 
           ...
           type = "lower",           # 只显示下三角
           lab_size = 3,             # 调整标签字体大小
           colors = c("#FFFAF0", "#FF5733", "#C70039"),  # 使用相同的渐变颜色
           title = "Correlation Matrix",  # 标题
           ggtheme = theme_minimal(),   # 使用简洁的主题
           method = "square")        # 使用方形网格
